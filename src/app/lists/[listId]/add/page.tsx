                ...
                <p className="mt-4 text-gray-500"> No games yet. Click "Add Game" to add your first game</p> // If game list empty, display this prompt
                    ...
                    <ul className="mt-6 space-y-3"> {/* List container */}
                        <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:gird-cols-7 gap-4">
                            { library.map((entry) => {
                                const alreadyInList = gameIdsInlist.has(entry.gameId);
                                const cover = entry.game.coverUrl ? igdbResizeImageUrl(entry.game.coverUrl, "t_cover_big") : null; 
                                    
                                return (  //loop through eahc library entry
                                <li key={entry.id} className="p-4"> {/* Each entry section */}
                                            ...
                                            <div>
                                                {alreadyInList ? ( //If this entry is already in this list, replace the button to add it to list with span message
                                                    <span className="text-sm text-gray-500">Already in list</span>
                                                        ...
                                                        name="gameId" 
                                                        value={entry.gameId} 
                                                        className="rounded border-1 px-3 py-1 text-sm text-white cursor-pointer" >
                                                        Add Game
                                                    </button>
                                                )}
                                            </div>                                                                                              
